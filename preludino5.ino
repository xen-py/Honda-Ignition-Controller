...
// Pin assignments
const int IGN_IN = 2;    // A21 from P28 ECU (logic-level, susceptible to coil noise)
const int CYP_IN = 3;    // B11/B12 MVR sensor (analog output, needs shielding)
...
// Prevents thrashing from noise-induced spurious CYP pulses
#define CYP_RESET_LOCKOUT 500000  // microseconds (500ms)
...
volatile unsigned long cyp_reset_lockout_count = 0;  // Debug: count locked-out pulses
...
void setup() {
    ...
    EICRA |= (1 << ISC00);                 // INT0: any change (IGN/A21)
    EICRA |= (1 << ISC11);                 // INT1: falling edge (CYP)
    
    EIMSK |= (1 << INT0) | (1 << INT1);

    Serial.begin(9600);
    Serial.println("\n=== Ignition Controller (Hysteresis + MVR Noise Immunity) ===");
    Serial.println("Waiting for CYP sync confirmation (3 pulses)...");
    Serial.println("CYP debounce: 15ms | IGN debounce: 2ms | CYP reset lockout: 500ms");
    
    sei();  // Enable interrupts globally
}

// CYP (crankshaft position) sync - MVR sensor on B11/B12 from distributor
// *** ELECTRICAL RECOMMENDATIONS FOR NOISE IMMUNITY ***
// 1. Route B11/B12 in shielded twisted pair, separate from coil return wires
// 2. Add 0.1µF ceramic capacitor from CYP pin to GND (RC low-pass filter ~1.6 kHz)
// 3. Keep PCB traces: CYP away from coil high-current return by >50mm
// 4. Star-point grounding: Arduino GND connects to ECU GND at single point
// 5. Use ferrite beads on A21 and CYP wires if noise persists
ISR(INT1_vect) {
    cyp_isr_count++;
    ...
    // Debounce: ignore pulses < 15ms apart
    // At 8000 RPM: edge spacing is 3.75ms
    // 15ms blocks MVR sensor ringing AND capacitive coupling artifacts
    if (elapsed < 15000) return;
    cyp_debounce_pass_count++;
    
    // ============= HYSTERESIS GATE =============
    // Prevent CYP resets from thrashing the sequence due to noise-induced spurious pulses.
    // Only allow resets if 500ms has passed since last reset.
    // Once established, the sequence order NEVER changes, so we can safely wait.
    unsigned long time_since_last_reset = now - last_cyp_reset_time;
    if (time_since_last_reset < CYP_RESET_LOCKOUT) {
        // Still in lockout window - ignore this CYP pulse for sequencing
        cyp_reset_lockout_count++;
        // But still count it and check sync (maybe noise, maybe real pulse)
        if (cyp_sync_count < MIN_CYP_FOR_SYNC) {
            // We're still confirming sync, so accept this pulse anyway
            cyp_sync_count++;
            if (cyp_sync_count == MIN_CYP_FOR_SYNC) {
    ...
    if (!dwell_active) {
        ...
        pending_sync = false;
        last_cyp_reset_time = now;  // Mark reset for hysteresis lockout
    } else {
...
// IGN signal - P28 ECU A21 output (logic-level, capacitively coupled from coil discharge)
// *** ELECTRICAL RECOMMENDATIONS FOR NOISE IMMUNITY ***
// 1. Route A21 in shielded twisted pair, separate from power return
// 2. Add 0.1µF ceramic capacitor from IGN pin to GND (RC low-pass filter ~1.6 kHz)
// 3. Shield should connect to Arduino GND ONLY (not ECU ground separately)
// 4. Ferrite bead on A21 wire if spurious edges persist during coil discharge
// 5. Ensure dwell time is long enough (>2ms) for coil charge stabilization
ISR(INT0_vect) {
    // Gate: require startup sync confirmation before firing
    ...
    // At 8000 RPM, edge spacing is 3.75ms, leaving 1.75ms margin
    ...
    if (state == LOW) {
        // ===== DWELL PHASE =====
        // P28 pulls A21 low to begin coil charging
        log_event(EVT_IGN_LOW, seq_snapshot);
        ign_state_low_count++;
        dwell_active = true;
        
        // Enable coil (via PC817: LOW on Arduino = HIGH on coil)
        if (active_pair_A) {
            PORTD &= ~(1 << COIL_A);
        } else {
            PORTD &= ~(1 << COIL_B);
        ...
        // ===== FIRE PHASE =====
        // P28 releases A21 high to trigger coil discharge (spark!)
        log_event(EVT_IGN_HIGH, seq_snapshot);
        ign_state_high_count++;
        
        // Disable coil (HIGH on Arduino = LOW on coil)
        if (active_pair_A) {
            PORTD |= (1 << COIL_A);
        } else {
            PORTD |= (1 << COIL_B);
...
void loop() {
    ...
    if (DEBUG_MODE && !sync_established) {
        ...
        if (!debug_printed) {
            Serial.println("DEBUG_MODE: Blind pair alternation (no CYP sync)");
            debug_printed = true;
        }
    }
