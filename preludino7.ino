...
volatile uint8_t ign_since_cyp = 0;
volatile bool cyp_armed = true;
// CYP (cylinder position) sync - MVR sensor condition by hei 4 pin module
ISR(INT1_vect) {
    cyp_isr_count++;
    ...
    if (!cyp_armed) return;
    if (ign_since_cyp < 3 && sync_established) return;//sync
    cyp_armed = false;
