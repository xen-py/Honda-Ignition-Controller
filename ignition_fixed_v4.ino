...
void log_event(uint8_t type, bool pair, uint8_t seq_snapshot) {
    history[history_head] = {type, micros(), pair, seq_snapshot};
...
ISR(INT1_vect) {
    ...
    bool pair_snapshot = active_pair_A;
    log_event(EVT_CYP, pair_snapshot, seq_snapshot);
    ...
    if (!dwell_active) {
        ...
        // Set to 3 so next INT0 increment wraps to 0 (Pair A)
        ign_sequence_count = 3;
        pending_sync = false;
    } else {
...
ISR(INT0_vect) {
    ...
    // Debounce: ignore edges < 2ms apart
    ...
    if (state == LOW) {
        // ===== DWELL PHASE =====
        // Start charging coil
        log_event(EVT_IGN_LOW, active_pair_A, seq_snapshot);
        ign_state_low_count++;
        dwell_active = true;
        ...
        log_event(EVT_IGN_HIGH, active_pair_A, seq_snapshot);
        ...
        // This is where pending sync (deferred from CYP during dwell) is applied.
        
        cli();  // Disable interrupts for critical section
        dwell_active = false;
        if (pending_sync && !DEBUG_MODE) {
            // Sync pulse arrived during dwell - apply it now
            ign_sequence_count = 0;    // Force Pair A on next fire
            pending_sync = false;
        } else {
            // Normal progression
            ign_sequence_count++;
